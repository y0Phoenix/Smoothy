...
pub async fn send_embed(
    generics: &Generics,
    embed: CreateEmbed,
    millis_dur: Option<u64>,
) -> Option<Box<Message>> {
    let builder = CreateMessage::new().embed(embed);
    match generics
        .channel_id
        .send_message(generics.http().await, builder)
        .await
    {
        Ok(msg) => {
            let msg = Box::new(msg);
            if let Some(dur) = millis_dur {
                generics
                    ...
                    .inner
                    .client_tx
                    .send(ClientChannel::DltMsg(DltMsg {
                        msg,
                        duration: dur,
...
pub async fn delete_now_playing_msg(meta_data: &TrackMetaData) {
    if let Some(now_playing_msg) = &meta_data.song.now_playing_msg.clone() {
        let http = meta_data
            ...
            .await
            .clone()
            .expect("Should be an Http initialized");
        let channel_id = ChannelId::from_str(now_playing_msg.channel_id.as_str())
            .expect("Should be valid ChannelId");
        if let Ok(msg) = channel_id
            .message(
                &http,
                MessageId::from_str(&now_playing_msg.msg_id).expect("Should be a valid MessageId"),
            )
            .await
        {
