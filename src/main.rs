...
use commands::{
    ...
    play::play, queue::queue, restart::restart, resume::resume, seek::seek,
};
use common::{
    embeds::LEAVING_COLOR,
    message::{DltMsg, SmMsg},
    server::{ServerGuildId, Servers},
    ClientChannel, DcTimeOut, SmData, UserData,
};
use dotenv::dotenv;
...
async fn main() {
    ...
    let test = env::var("TEST")
        .expect("Expected a TEST bool in the environment")
        .parse::<bool>()
        .expect("Expected TEST environment variable to be a bool");
    ...
    let mut dc_timers: std::collections::HashMap<ServerGuildId, DcTimeOut> =
    ...
    loop {
            ...
            if msg.timer.just_finished() {
                ...
                if (msg.msg.delete(http.clone()).await).is_err() {
                    let server_name: String = match msg.msg.guild(&cache_clone_main) {
                        ...
                        None => {
                            warn!(
                                "Error while trying to get guild from Message {}",
                                msg.msg.id
                            );
                            continue;
                        }
                ...
                if in_vc {
                    ...
                    send_msg(
                        SmMsg {
                            ...
                            content: CreateEmbed::new()
                                .color(LEAVING_COLOR)
                                .description(":cry: Left vc due to idle"),
                        },
                        &http,
                        Some(30000),
                        &mut dlt_msgs,
        ...
        match client_rx.recv_timeout(Duration::from_millis(5)) {
            Ok(from_client) => match from_client {
                ClientChannel::DcTimeOut(dc_timout) => {
                    match dc_timers.get(&dc_timout.guild_id) {
                        Some(_dc_timer) => {
                            if dc_timout.end {
                                dc_timers
                                    .remove(&dc_timout.guild_id)
                                    .expect("Should be able to remove dc timer");
