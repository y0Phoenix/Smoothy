use crate::{
    common::{
        checks::is_playing,
        embeds::{err_embed, GET_SERVER_FAIL_DLT_TIME, GET_SERVER_FAIL_MSG, LOOPED_COLOR},
...
pub async fn loopqueue(ctx: SmContext<'_>) -> CommandResult {
    ...
    let manager = generics
        .data
        .inner
        .songbird
        .get(generics.guild_id)
        .expect("Should be a valid 'Call'");
    let manager_lock = manager.lock().await;
    let track = manager_lock
        .queue()
        .current()
    ...
    let meta_data = typemap
        ...
        .expect("Track should have meta data");
    ...
    if meta_data.looped {
        if track.disable_loop().is_err() {
            send_embed(
                &generics,
                err_embed(
                    ":cry: Failed to disable song loop before looping queue. Try again later",
                ),
                Some(120000),
            )
            .await;
