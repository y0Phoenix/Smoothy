    ...
    async fn act(&self, ctx: &EventContext<'_>) -> Option<Event> {
        if let EventContext::Track(tracks) = ctx {
            ...
            if server.songs.0.looped {
                let manager = meta_data
                    .generics
                    .data
                    .inner
                    .songbird
                    .get(meta_data.generics.guild_id)
                    .expect("Should be a valid manager");
                let mut manager_lock = manager.lock().await;

                let track = init_track(
                    meta_data.src.clone(),
                    &meta_data.generics,
                    SongType::DB(meta_data.song.clone()),
                    &mut manager_lock,
                )
                .await
            ...
            if no_more_songs {
                ...
                info!(
                    "No more songs to play in {} starting dc timeout",
                    server.name
                );
                meta_data.generics.data.inner.start_dc_timer(
